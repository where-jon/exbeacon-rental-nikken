...
class BeaconService @Inject() (config: Configuration, ws: WSClient, carDAO: models.itemCarDAO,@Named("SaveBtxDataActor") excIfActor: ActorRef,exbDao:models.ExbDAO) extends Controller {
  ...
  /**
    ...
    * その際、btx_masterテーブルと結合してbeaconPositionDataのリストとして返却する
    *
    * @param blankInclude  ブランクレコード（名前=param01が空）を含めるかどうか
    * @return  List[beaconPositionData]
    */
  def getBeaconPosition(blankInclude: Boolean = false, placeId:Int): Seq[beaconPositionData] = {
   ...
   // val posList = Await.result(f, timeout.duration).asInstanceOf[List[beaconPosition]]
    ...
    val bplist = dbDatas.map { v =>
      ...
      if (bpd.isDefined && blankTargetMode) {
        ...
        beaconPositionData("現在フロア","現在位置",-1, -1, -1, 0, "nodate",
          v.item_car_id,
          v.item_car_btx_id,
          v.item_car_key_btx_id,
          v.item_type_id,
