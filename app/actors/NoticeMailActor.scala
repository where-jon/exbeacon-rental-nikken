...
class NoticeMailActor @Inject()(config: Configuration
  ...
  private def execute():Unit = {
    if (enableLogging && noticeDaysAgo != 0) {
      // noticeDaysAgoの最小値は土日を考えると４日前が良いので環境変数は4日～20日としているが、
      // 但し平日の場合は最小日を1日でチェックする。
      //　※ noticeDaysAgo >= MinimumDate
      // メール送信日の曜日を取得
      ...
      if (noticeDaysAgo >= MinimumDate && noticeDaysAgo <= 20) {
  ...
  def checkMailAddress(address: String, regularExpression: String): Boolean = {
    ...
    if (matcher.find) {
      ...
      Logger.error(s"""${address}は不正なメールアドレス""")
      false
    }
  }
}
