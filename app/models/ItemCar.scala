...
class itemCarDAO @Inject()(dbapi: DBApi) {
  ...
  def selectCarMasterSearch(
    ...
    db.withConnection { implicit connection =>
      ...
      if(workTypeName == "終日" ){
        ...
        wherePh +=
          s"""
              and r.reserve_start_date = to_date('${reserveDate}', 'YYYY-MM-DD') and not work.work_type_name = '${workTypeName}'  """
      }
  ...
  def selectCarMasterSearchBest(
    ...
    db.withConnection { implicit connection =>
      ...
      var vCount = this.getCarMasterSearchBest(placeId, itemTypeId, workTypeName, reserveDate, itemIdList).length
